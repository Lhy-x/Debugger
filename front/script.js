...
async function saveScore() {
  ...
  try {
    ...
    Swal.fire({
      icon: "success",
      title: "Score sauvegardé avec succès",
      showConfirmButton: false,
      timer: 1500
    });
...
async function fetchScores() {
  try {
    const response = await fetch(`http://localhost:3000/scores?difficulty=${difficulty}`);
    if (!response.ok) {
      throw new Error("Erreur lors de la récupération des scores");
    }
    const scores = await response.json();
    if (!Array.isArray(scores)) {
      throw new Error("Les scores récupérés ne sont pas un tableau");
    }
...
function displayScores(scores) {
  ...
  difficultyTitle.classList.add("text-2xl", "font-bold", "mb-4");
  difficultyTitle.textContent = `Scores pour la difficulté : ${difficulty.charAt(0).toUpperCase() + difficulty.slice(1)}`;
  scoreList.appendChild(difficultyTitle);

  scores.forEach((score, index) => {
    const listItem = document.createElement("li");
    listItem.classList.add("mb-2", "font-bold", "text-xl");

    if (index === 0) {
      const emoji = document.createElement("i");
      emoji.classList.add("fas", "fa-trophy", "text-yellow-500");
